...
ZiftLab: the official site/platform for a digital agency. `FASES.MD` (repo root) is the authoritative master spec/roadmap (FASE 0–16). Current state: **FASE 12 (analytics/conversion) complete** (FASE 7, 8, 9 y 16 ya completadas; 12 y 16 done out of order) — next up is FASE 10 (security/hardening), then FASE 11 (performance). FASE 12 pieces: `web/src/components/Analytics.astro` loads GA4/Clarity **only after** analytics consent (basic Consent Mode; IDs in `web/.env` `PUBLIC_GA4_ID`/`PUBLIC_CLARITY_ID`, empty in dev → nothing injected) and owns the delegated click listener for `[data-track-cta]`/`[data-track-whatsapp]` attributes (Button.astro accepts them as props); `web/src/lib/analytics.ts` `track()` pushes in gtag `arguments` format (plain-object pushes are GTM syntax — gtag.js ignores them); `web/src/lib/utm.ts` captures first-touch UTMs per session (sessionStorage `ziftlab-utm`, clean keys) which travel with leads (group `utm` in the leads collection, mirrored in `cms/src/leads/schema.ts`) and with Calendly bookings (`asesoria.astro` rewrites the widget `data-url` via a `set:html` inline script — Prettier's Astro parser chokes on inline scripts with content inside JSX ternaries, use the `set:html` template-string pattern like BaseLayout's `consentInit`). Event taxonomy, GA4/funnel/Looker Studio setup and UTM naming live in `ANALITICA.md`. FASE 8 pieces: landings SEO locales in the `landings` collection rendered by `web/src/pages/[slug].astro` (reserved-slug filter inside its `getStaticPaths`), JSON-LD builders in `web/src/lib/schema.ts` + `JsonLd.astro`/`Breadcrumbs.astro`, robots at `web/src/pages/robots.txt.ts`, and 301s read from the CMS `redirects` collection by `astro.config.mjs` at config-load time (CMS must be up; dev server restart picks up new redirects). Consent state lives in `web/src/lib/consent.ts` + the inline Consent Mode script in `BaseLayout.astro`. MinIO must be running for media uploads (`./infra/scripts/start-minio.sh`; bucket via `./infra/scripts/create-minio-bucket.sh`). The Payload CMS must be running for `pnpm dev:web` / `pnpm build:web` to render real data (the web fetches at build/request time).
...
- `cms/` — Payload CMS v3 on Next.js 16 (Turbopack), Postgres via `@payloadcms/db-postgres`. Admin + content API only — it must never render the public site. Config lives in `cms/src/payload.config.ts`; collections in `cms/src/collections/`, globals in `cms/src/globals/`, access helpers in `cms/src/access/`, reusable field factories (slug, link/CTA, social links) in `cms/src/fields/`, seed in `cms/src/seed/`. Content collections with an editorial flow (services, projects, posts, home-page global) use `versions.drafts` + the `publishedOnly` read helper; SEO `meta` fields come from `@payloadcms/plugin-seo`. Env vars (in `cms/.env`, never committed): `DATABASE_URL`, `PAYLOAD_SECRET` — note the template uses `DATABASE_URL`, not `DATABASE_URI` as written in docs/FASES.MD §11. `payload run` scripts (e.g. the seed) only read `cms/.env`, not the repo root. Public lead intake (FASE 7) goes only through `POST /api/leads/submit` (`cms/src/leads/`: Zod schema + honeypot + in-memory IP rate limit + notification/confirmation emails); direct create on the leads collection requires auth. Email is `@payloadcms/email-resend` gated on `RESEND_API_KEY` (unset in dev → Payload logs emails to console); the client-side Zod mirror lives in `web/src/lib/leads.ts` and must stay in sync with `cms/src/leads/schema.ts`.
